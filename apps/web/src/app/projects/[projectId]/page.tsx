  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] px-8 py-10">
      <div className="max-w-4xl mx-auto">
        ...
        {versions === null && !error && <p className="text-[var(--text-secondary)] text-sm">Loading...</p>}
          ...
          {versions?.map((v) => (
            ...
            >
              <div>
                ...
                <p className="text-[var(--text-muted)] text-xs">{v.document_title}</p>
              </div>
              <span className="text-[var(--text-muted)] text-xs">{new Date(v.created_at).toLocaleString()}</span>
            </Link>
          ))}
        </div>
      </div>
